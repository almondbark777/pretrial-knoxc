...
> ⚠️ PII note: this route emails defendant data to the mailbox you choose. Use a
> dedicated mailbox, restrict who can read it, and treat it as sensitive. The
> in-tenant alternative (Power Automate premium HTTP straight to ptr1, or an IT
> app registration) keeps the data inside Microsoft — prefer that for production.
...
Copy the updated webapp (which now includes `sharepoint_import.py`) to ptr1 and
install the timer. From your workstation:

```powershell
scp "C:\Users\alexa\fuck around with claude code\pretrial-knoxc\webapp\sharepoint_import.py" alex@100.85.252.79:~/
```
